...
//wifiSimp::wifiSimp(void);

void wifiSimp::init(ESP8266 wifi,char* SSID,char* PASSWORD){
    Serial.begin(9600);
...
//void wifiSimp::wifiWrite(char* A) wifi.send(mux_id, (uint8_t*) A, sizeof(A) - 1);
#define wifiWrite(A) wifi.send(mux_id, (uint8_t*) A, sizeof(A) - 1);

void wifiSimp::sendServer(ESP8266 wifi){
